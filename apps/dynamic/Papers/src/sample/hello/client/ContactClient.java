...
public class ContactClient {
	...
	public static void getOneContact(WebResource r, String id) {
		...
		ClientResponse response  = r.path(id).accept(MediaType.APPLICATION_XML).get(ClientResponse.class);
		
		JAXBElement<Contact> jaxbContact = response.getEntity(generic);
		Contact contact = jaxbContact.getValue();
		System.out.println("[derek]" + contact.getId() + ": " + contact.getName());
	}
	...
	public static void putOneContact(WebResource r, Contact c) {
		...
		System.out.println(response.getStatus()+"derek mark!");
	}
